...
class GaugeWidget(QWidget):
    ...
    START_ANGLE = 225    # degrees (Qt: 0=3 o'clock, CCW)
    SPAN_ANGLE  = 270    # degrees of arc
    ...
    def __init__(self, min_val=0.0, max_val=100.0, unit="",
        ...
        self._crit   = crit
        self._value  = min_val
        self._filled = 0.0      # 0.0–1.0 arc fill fraction
        self._label  = "—"
    ...
    def paintEvent(self, event):
        ...
        if self._filled > 0:
            ...
            p.drawArc(rect,
                      int(self.START_ANGLE * 16),
                      int(-self._filled * self.SPAN_ANGLE * 16))

        # Value text
        p.setPen(QColor("#00d4ff"))
        font = QFont("Courier New", 11, QFont.Weight.Bold)
        p.setFont(font)
        p.drawText(int(cx - 40), int(cy - 8), 80, 20,
        ...
        p.setFont(font2)
        p.drawText(int(cx - 30), int(cy + 10), 60, 16,

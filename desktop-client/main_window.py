"""
MORBION SCADA Desktop — Main Window
QMainWindow. Header with logo + status. Tab bar. All views.
Wires WS thread to all views. Wires REST client to control panels.
"""
...
from PyQt6.QtWidgets import (
    ...
    QTabWidget, QLabel, QStatusBar, QSizePolicy, QFrame
)
...
from theme import STYLESHEET, C_ACCENT, C_RED, C_GREEN, C_MUTED, C_TEXT2
...
class HeaderWidget(QWidget):
    ...
    def __init__(self, config: dict, parent=None):
        ...
        sub = QLabel("SCADA v3.0  ·  INDUSTRIAL CONTROL SYSTEM")
        sub.setStyleSheet(f"color:{C_MUTED};font-size:8px;letter-spacing:3px;")
        ...
        for w in (self._lbl_processes, self._lbl_alarms,
                  self._lbl_poll, self._lbl_time):
            stats_layout.addWidget(w)

        # Connection dot
        self._conn_dot = QLabel("●")
        self._conn_dot.setStyleSheet(f"color:{C_MUTED};font-size:14px;")
        self._conn_lbl = QLabel("CONNECTING")
        self._conn_lbl.setStyleSheet(
            f"color:{C_MUTED};font-size:9px;letter-spacing:2px;")
        stats_layout.addWidget(self._conn_dot)
        stats_layout.addWidget(self._conn_lbl)
        layout.addLayout(stats_layout)

    def _stat(self, text: str) -> QLabel:
        lbl = QLabel(text)
        lbl.setStyleSheet(f"color:{C_TEXT2};font-size:9px;letter-spacing:2px;font-family:'Courier New';")
        return lbl
    ...
    def set_connected(self, connected: bool):
        if connected:
            self._conn_dot.setStyleSheet(f"color:{C_GREEN};font-size:14px;")
            self._conn_lbl.setText("LIVE")
            self._conn_lbl.setStyleSheet(
                f"color:{C_GREEN};font-size:9px;letter-spacing:2px;")
        else:
            self._conn_dot.setStyleSheet(f"color:{C_RED};font-size:14px;")
            self._conn_lbl.setText("RECONNECTING")
            self._conn_lbl.setStyleSheet(
                f"color:{C_RED};font-size:9px;letter-spacing:2px;")

    def update_plant(self, plant: dict):
        online  = sum(1 for k in ("pumping_station","heat_exchanger","boiler","pipeline")
        ...
        alarm_color = C_RED if n_crit > 0 else (C_TEXT2 if len(alarms)==0 else "#ffcc00")
        self._lbl_alarms.setText(f"{len(alarms)} ALARMS")
        self._lbl_alarms.setStyleSheet(
            f"color:{alarm_color};font-size:9px;letter-spacing:2px;font-family:'Courier New';")

        self._lbl_poll.setText(f"POLL #{plant.get('poll_count', 0)}")
        ts = plant.get("server_time", "")
        self._lbl_time.setText(ts.split(" ")[1] if " " in ts else ts)
...
class MorbionMainWindow(QMainWindow):
    ...
    def __init__(self, config: dict):
        ...
        self.setWindowTitle(ui.get("window_title", "MORBION SCADA v3.0"))
        self.resize(
        ...
        self._status.showMessage(
            "MORBION SCADA v3.0  ·  Intelligence. Precision. Vigilance.")
    ...
    def _on_plant_data(self, plant: dict):
        """Called on every WS push. Update ALL views — always."""
        self._plant = plant
        self._header.update_plant(plant)

        # Update all views unconditionally — this is the fix for static data
        # All widgets exist in DOM (Qt tree) regardless of which tab is active
        try:
            self._ov_view.update_data(plant)
            self._pump_view.update_data(plant.get("pumping_station", {"online": False}))
            self._hx_view.update_data(plant.get("heat_exchanger", {"online": False}))
            self._boiler_view.update_data(plant.get("boiler", {"online": False}))
            self._pipeline_view.update_data(plant.get("pipeline", {"online": False}))
            self._alarms_view.update_data(plant)
        except Exception as e:
            log.error("View update error: %s", e)

        # Update tab alarm badge
        n_alarms = len(plant.get("alarms", []))
        alarm_tab_text = f"ALARMS ({n_alarms})" if n_alarms > 0 else "ALARMS"
        self._tabs.setTabText(5, alarm_tab_text)

    def _on_connection(self, connected: bool):
        self._header.set_connected(connected)
        if connected:
            self._status.showMessage("● LIVE — Connected to MORBION SCADA Server")
        else:
            self._status.showMessage("⚠ RECONNECTING — Lost connection to server...")
